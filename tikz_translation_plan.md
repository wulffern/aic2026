...
- After approval, switch only that lecture reference to `../media/<basename>_tikz.pdf`.
...
4. Compile only that figure with `pdflatex` to `tikz/build/<basename>.pdf`.
5. Copy outputs to:
   - `media/<basename>_tikz.pdf`
   - `pdf/media/<basename>_tikz.pdf`
   - optional SVG mirrors if the existing conversion path succeeds cleanly
6. Open:
   - original SVG/PDF source
   - proposed TikZ PDF
7. Report the proposal briefly and stop.

Approval gate:
- `approve`: update the lecture reference if needed, then move to the next queued figure
- `comment`: revise the same figure, recompile, reopen both files, and wait again
- never start the next figure without explicit approval
...
- generated artifacts exist in both `media/` and `pdf/media/`

## Assumptions and Defaults
- Continue the queue from `l3_vsrc` first.
...
## Saved Progress
- Approved and switched in `lectures/l03_refbias.md`:
  - `l3_vsrc`
  - `l3_isrc`
  - `l3_bjtonly`
- Current next figure:
