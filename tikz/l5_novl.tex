...
\begin{circuitikz}[american, transform shape]
  ...
  % The clock rail sits at the gate's own input height, so it arrives without a
  % bend: a right-angle spur into the side of a gate reads as a second input.
  \coordinate (CK) at (0.3,0 |- N1.in 1);
  \coordinate (CKT) at (1.3,0 |- N1.in 1);
  \novlPort{CK}{east}{$\mathrm{Clk}$\;}
  \draw[thick] (CK) -- (N1.in 1);
  \fill (CKT) circle (0.07);
  \node[not port, rotate=-90] (INV) at (1.3,1.0) {};
  \draw[thick] (CKT) -- (INV.in 1);
