...
\node[nor port] (rst) at (3.9,2.3) {};
\draw (upN) to [short,*-] ++(0,-1.0) -| (rst.in 1);
\draw (dwnN) to [short,*-] ++(0,1.0) -| (rst.in 2);
\draw (rst.out) -- ++(0.25,0) coordinate (cfb);
\draw (cfb) |- (1.6,3.0);
\draw (cfb) |- (1.6,-0.6);
\fill (cfb) circle (0.075);
\node [anchor=west] at (4.85,2.3) {CFB};

% Output inverters: UP_N -> UP -> CP_UP_N, DOWN_N -> CP_DOWN
\draw (upN) \hinv{U1} \hinv{U2};
\draw (U2.out) \portOut{$\overline{CP_{UP}}$};
\draw (dwnN) \hinv{D1};
\draw (D1.out) \portOut{$CP_{DOWN}$};
