...
\begin{circuitikz}[american, thick, transform shape, circuit ee IEC]
  ...
  % The wide-swing master, wired as the original: node A at the top of
  % the NMOS stack is the diode node - the upper device's gate ties to
  % its own drain - and the LOWER device's gate ties to that same node,
  % which is what makes it sit at the edge of saturation. VBN is node A.
  % The VBN rail crosses this branch's own column without a dot.
  \draw (6.0,4.6) -- (6.0,4.0);
  \fill (6.0,4.0) circle (0.075);
  \draw (6.0,2.4) to [Tnmos, n=mnd1] ++(0,\grid);
  \dsize{4.55}{2.8}{24F2F\\M=2}
  %- the diode: this gate to this drain
  \draw (5.02,3.2) -- (4.85,3.2) -- (4.85,4.0) -- (6.0,4.0);
  \draw (6.0,1.6) -- (6.0,2.4);
  \draw (6.0,0) to [Tnmos, n=mnd2] ++(0,\grid);
  \dsize{6.15}{0.45}{24F4F\\M=1}
  %- the lower gate hangs on the same node, and VBN leaves from it
  \draw (4.3,4.0) -- (4.3,0.8) -- (5.02,0.8);
  \draw (4.85,4.0) -- (4.3,4.0);
