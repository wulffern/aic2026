...
\begin{circuitikz}[american, thick, transform shape, circuit ee IEC]
  ...
  % ================= VBP branch: the PMOS wide-swing master ========
  % Same trick as the NMOS master, upside down: the mirror device on
  % top has its gate on the BOTTOM of the stack - the cascode's drain -
  % and that node is VBP. The cascode between them is gated by VCP.
  ...
  %- the mirror gate hangs on the bottom of the stack, and VBP is that
  %- node. The rail crosses the branch's own column without a dot.
  \fill (3.2,3.6) circle (0.075);
  \draw (3.2,3.6) -- (1.7,3.6) -- (1.7,7.2) -- (2.22,7.2);
  \fill (1.7,6.3) circle (0.075);
  \draw (1.7,6.3) -- (15.4,6.3) to [short,-o] ++(0.5,0) node[anchor=west] {$VBP$};
  ...
  % The wide-swing cascode master. The reference is mirrored once in a
  % plain mirror (the R line along the bottom); everything above it is
  % wide-swing cascode. In this master the LOWER device's gate ties to
  % the top of the stack - that node is VBN - while the cascode above
  % it is gated by VCN, which the narrow 8F12F device generates. Tying
  % both gates to the node instead would be the simple cascode mirror,
  % and would cost the swing this circuit exists to keep.
  \draw (6.0,4.6) -- (6.0,4.0);
  \fill (6.0,4.0) circle (0.075);
  \draw (6.0,2.4) to [Tnmos, n=mnd1, mirror] ++(0,\grid);
  \dsize{4.45}{2.9}{24F2F\\M=2}
  %- the cascode gate comes from the VCN rail
  \draw (6.98,3.2) -- (6.98,3.95);
  \draw (6.0,1.6) -- (6.0,2.4);
  \draw (6.0,0) to [Tnmos, n=mnd2] ++(0,\grid);
  \dsize{6.15}{0.45}{24F4F\\M=1}
  %- the mirror device's gate hangs on the top node, and that node is
  %- VBN. The rail crosses this branch's own column without a dot.
  \draw (6.0,4.0) -- (4.3,4.0) -- (4.3,0.8) -- (5.02,0.8);
  \fill (4.3,2.3) circle (0.075);
  \draw (4.3,2.3) -- (15.4,2.3) to [short,-o] ++(0.5,0) node[anchor=west] {$VBN$};
