...
\begin{tikzpicture}[thick]
  ...
  \foreach \y/\tag/\sg in {\yi/i/1, \yq/q/-1} {
    ...
    \node[scale=0.8, anchor=south] at (7.6,\y+0.1) {4b};
  ...
  %- the other path's summing node. Two separate verticals so they do not
  %- run on top of each other in the middle.
  \draw[red, thick] (1.4,\yi) -- (1.4,0.85) -- (-2.35,0.85);
  \draw[red, thick, ->] (-2.35,0.85) -- (-2.35,{\yq+0.22});
  \fill[red] (1.4,\yi) circle (0.075);

  \draw[red, thick] (1.4,\yq) -- (1.4,-0.85) -- (-2.05,-0.85);
  \draw[red, thick, ->] (-2.05,-0.85) -- (-2.05,{\yi-0.22});
