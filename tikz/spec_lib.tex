...
\definecolor{bandorange}{rgb}{0.95,0.62,0.10}
\definecolor{annotblue}{rgb}{0.15,0.30,0.85}
...
\newcommand{\specAxis}[1]{%
  \draw[axisblue, thick, <->] (-\specXmax,#1) -- (\specXmax,#1);
  \draw[axisblue, thick, ->] (0,#1) -- (0,#1+\specVh);
  \foreach \p/\lbl in {-2/{-f_{s}}, -1/{-f_{s}/2}, 1/{f_{s}/2}, 2/{f_{s}}} {
    \draw[axisblue, thick] (\p*\specHalf,#1+0.13) -- (\p*\specHalf,#1-0.13);
    \node[axisblue, anchor=north] at (\p*\specHalf,#1-0.22) {$\lbl$};
  }
}
...
\newcommand{\specBand}[4]{%
  \draw[bandorange, very thick, rounded corners=3pt]
    (#1-#2-0.30,#4) -- (#1-#2,#4+#3) -- (#1+#2,#4+#3) -- (#1+#2+0.30,#4);
}

% A block in the signal chain: left edge x, centre y, label.
\newcommand{\specBlock}[3]{%
  \draw[axisblue, thick] (#1,#2-0.5) rectangle ++(1.5,1.0);
  \node[axisblue] at (#1+0.75,#2) {#3};
}

% An arrow between chain elements: from x, to x, at y.
\newcommand{\specWire}[3]{\draw[axisblue, thick, ->] (#1,#3) -- (#2,#3);}
