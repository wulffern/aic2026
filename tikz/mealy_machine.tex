...
% Mealy state machine: the output combinatorics see both the state and
% the inputs, so input glitches can propagate straight to the output.
...
\begin{tikzpicture}[thick, color=blue,
  ...
  % input bus: feeds first COMB and (Mealy!) the output COMB directly
  \node[anchor=south east] at (-1.9,2.15) {in};
  \draw (-2.1,2.1) -- (6.8,2.1);
  \draw[a] (0,2.1) -- (c1.north);
  \draw[a] (6.8,2.1) -- (c2.north);
