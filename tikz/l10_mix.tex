...
\begin{circuitikz}[american, thick, transform shape, circuit ee IEC]
  ...
  \newcommand{\mixsw}[3]{%
    ...
    \draw (#3.S) -- (0,#1);
    \draw (#3.D) -- (2.3,#1);
    \draw (2.3,#1) to [short,-o] (2.42,#1);
    % the AC coupling capacitor up to the clock port, with enough gate
    % lead that the plates clear the device instead of sitting on it
    \draw (#3.G) -- (0.85,{#1+1.45});
    \draw (0.55,{#1+1.45}) -- (1.15,{#1+1.45});
    \draw (0.55,{#1+1.65}) -- (1.15,{#1+1.65});
    \draw (0.85,{#1+1.65}) -- (0.85,{#1+2.1});
    \draw (0.85,{#1+2.1}) to [short,-o] (0.85,{#1+2.22});
    ...
    \node[anchor=west] at (1.06,{#1+2.22}) {#2};
    ...
    % reaches.
    \fill (0.85,{#1+1.2}) circle (0.075);
    \draw (0.85,{#1+1.2}) -- (1.5,{#1+1.2});
    \draw (1.5,{#1+1.2}) \hresistor{};
    \draw (3.1,{#1+1.2}) to [short,-o] (3.22,{#1+1.2});
    \node[anchor=west] at (3.38,{#1+1.2}) {$V_n$};
  }
  ...
  \draw (0,3.6) to [short,-o] (-1.42,3.6);
