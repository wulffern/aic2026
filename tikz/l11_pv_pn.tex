...
% region, where the minority carrier first diffuses (the wiggly leg)
% and only contributes if it reaches the depletion edge. All three
...
% is written K here, since the N side is the cathode.

\begin{circuitikz}[american, thick, transform shape]
  \input{tikz/ckt_lib.tex}
  \input{tikz/mos_lib.tex}  % echarge/hcharge, the carrier colours
