...
\begin{circuitikz}[american, thick, transform shape, circuit ee IEC]
  ...
  \newcommand{\dacCell}[3]{%
    ...
    % The drop to the rail is taken from beyond the end of the gate lead, off
    % the node's own anchor, so it cannot be laid across the device symbol.
    \draw let \p1 = (ML#2.gate) in
      (ML#2.gate) -- (\x1-0.35,\y1) -- (\x1-0.35,\ybias);
    \path let \p1 = (ML#2.gate) in (\x1-0.35,\ybias) coordinate (mlv#2);
    \fill (mlv#2) circle (0.07);
