...
\newcommand{\planeAxes}[2]{%
  \draw[axisblue, thick, ->] (-\planeXmax,0) -- (\planeXmax,0);
  \node[axisblue, anchor=north] at (\planeXmax-0.25,-0.22) {#1};
  \draw[axisblue, thick, <->] (0,-\planeYmax) -- (0,\planeYmax);
  \node[axisblue, anchor=west] at (0.22,\planeYmax-0.28) {#2};
}

% The unit circle, drawn heavy because it is the thing that matters.
\newcommand{\planeCircle}{\draw[axisblue, ultra thick] (0,0) circle (\planeR);}
