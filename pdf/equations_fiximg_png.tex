\textbf{Keywords:} Reference, Physics, Semiconductors, Diode, MOSFET,
Noise, Circuits, References, Filters, Switched Capacitor, Data
Converters, Regulators, PLL, Oscillators, Radio

Every equation of the lecture series in one place, ordered from
fundamental physics to the highest abstraction. No derivations and
almost no words --- those live in the lectures --- just the results, as
a reference.

\section{Fundamental physics}\label{fundamental-physics}\index{fundamental physics@Fundamental physics}

\textbf{The QED Lagrangian --- everything in electronics follows from
this}
...
\textbf{Probability density of a particle}
...
\textbf{Fermi-Dirac distribution, and its Boltzmann tail}
...
\textbf{Doped silicon (mass action)}
...
\textbf{Thermal voltage}

\[ V_T = \frac{kT}{q} \approx 25.85\text{ mV @ 300 K} \]

\section{Diode}\label{diode}\index{diode@Diode}

\textbf{Built-in voltage of a pn junction}

\[ \Phi_0 = V_T \ln\left(\frac{N_A N_D}{n_i^2}\right) \]

\textbf{Depletion width}
...
\textbf{Forward voltage temperature dependence}

\[ V_D = \frac{kT}{q}(\ell - 3 \ln T) + V_G \Rightarrow \frac{dV_D}{dT} = \frac{k}{q}\left(\ell - 3\ln T - 3\right) \]

\textbf{Difference of two diode voltages (PTAT)}

\[ V_{D1} - V_{D2} = V_T \ln N \]

\textbf{Generation (leakage) current of a reverse biased junction}

\[ I_{gen} = \frac{q A n_i W}{\tau_g} \]

\section{MOSFET}\label{mosfet}\index{mosfet@MOSFET}
...
\textbf{Transconductance efficiency}

\[ \frac{g_m}{I_D} = \frac{1}{nV_T} \text{ (weak)} \text{ , } \frac{g_m}{I_D} = \frac{2}{V_{eff}} \text{ (strong)} \]

\textbf{Output conductance and intrinsic gain}
...
\textbf{Mean square and power spectral density}

\[ \overline{x^2(t)} = \int_{0}^{\infty}{S_x(f)df} \text{ , } S_y(f) = S_x(f)\vert H(f)\vert ^2 \]

\textbf{Thermal noise of a resistor}

\[ S_{th}(f) = 4kTR \]

\textbf{Sampled (kT/C) noise bandwidth of an RC}

\[ f_x = \frac{\pi f_0}{2} = \frac{1}{4RC} \]

\textbf{Uncorrelated sources add in power}
...
\textbf{Noise factor and Friis' formula}
...
\textbf{Common drain (source follower)}
...
\textbf{Source degeneration and cascode output resistance}

\[ r_{out} = r_{ds2}\left[1 + R_s(g_{m2} + g_{ds2})\right] \approx r_{ds2}\, g_{m2} R_s \]

\section{References and bias}\label{references-and-bias}\index{references and bias@References and bias}

\textbf{Bipolar / diode core}

\[ V_{BE} = V_T \ln\frac{I_C}{I_S} \text{ (CTAT)} \text{ , } \Delta V_{BE} = V_T \ln N \text{ (PTAT)} \]

\textbf{Brokaw bandgap output}

\[ V_{REF} = V_{BE3} + \frac{R_2}{R_3} V_T \ln\frac{R_2}{R_1} \]

\textbf{Bandgap voltage with curvature}
...
\textbf{SC gain stage and integrator}

\[ H(z) = \frac{C_1}{C_2}z^{-1} \text{ , } H(z) = \frac{C_1}{C_2}\frac{z^{-1}}{1 - z^{-1}} \]

\textbf{First and second order IIR}

\[ H(z) = \frac{b}{z-a} \text{ , } H(z) = \frac{b z}{z^2 - 2a z + (a^2+b^2)} \]

\section{Data converters}\label{data-converters}\index{data converters@Data converters}
...
\textbf{First order noise shaping}
...
\textbf{DAC: a digital number scales a reference}

\[ V_{out} = D_{in} \times V_{ref} \]

\section{Voltage regulation}\label{voltage-regulation}\index{voltage regulation@Voltage regulation}

\textbf{The inductor and capacitor of a switcher}

\[ I_x(t) = \frac{1}{L}\int{V_x(t)dt} \text{ , } V_o(t) = \frac{1}{C}\int{(I_x(t) - I_o(t))dt} \]

\textbf{Ideal buck output}

\[ V_o = V_{in} \times \text{Duty-Cycle} \]

\section{PLL}\label{pll}\index{pll@PLL}

\textbf{A modulated carrier, and phase versus frequency}

\[ A_m(t)\cos\left(2\pi f_{c}t + \phi_{m}(t)\right) \text{ , } \phi(t) = 2\pi\int_0^t f(t)dt \]

\textbf{Loop gain of a charge-pump PLL}
...
\section{Oscillators}\label{oscillators}\index{oscillators@Oscillators}

\textbf{Crystal input impedance}

\[ Z_{in} \approx \frac{L C_F s^2 + 1}{L C_F C_P s^2 + C_F + C_P} \]

\textbf{Ring oscillator frequency}

\[ f = \frac{1}{2 N t_{pd}} \text{ , } t_{pd} \approx RC \Rightarrow f = \frac{\mu_n (VDD-V_{th})}{\frac{4}{3} N L^2} \]

\textbf{Current starved ring}

\[ f \approx \frac{I_{control}}{C \frac{VDD}{2} N} \]

\section{Radio}\label{radio}\index{radio@Radio}

\textbf{Friis transmission (free space)}

\[ P_{RX} = \frac{P_{TX}}{D^2}\left[\frac{\lambda}{4\pi}\right]^2 \]

\textbf{Receiver sensitivity}
...
Every equation here is derived in its own chapter of this book; the
section titles above point at them

Johns and Martin carry the same set with more algebra {[}1{]}
