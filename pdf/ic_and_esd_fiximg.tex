...
±0.54 °C (3-sigma) Inaccuracy from -40 °C to 125 °C {[}1{]}.
...
\subsubsection{Supply}\label{supply}
...
regulator (LDO) provide the digital supply (1.8 V).

See more at
\href{\%5BAbsolute\%20maximum\%20ratings\%5D(https://caravel-harness.readthedocs.io/en/latest/maximum-ratings.html)}{Analog
Specs}
...
level), then we need a reference, a compared and maybe other circuits.
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/por.pdf}
}

\emph{Figure 1: Power On Reset using gate tunneling }

\section{Electrostatic Discharge}\label{electrostatic-discharge}\index{Electrostatic Discharge}
...
few ESD books in existence, shown below, and rely on you understanding
of PN-junctions.
...
\subsection{Human body model (HBM)}\label{human-body-model-hbm}\index{Human body model (HBM)}

HBM is the ``simple'' version of ESD, a model can be seen in Figure 3.
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/esd_hbm_finger.pdf}
}
...
\subsection{Charged device model (CDM)}\label{charged-device-model-cdm}\index{Charged device model (CDM)}
...
\href{https://en.wikipedia.org/w/index.php?title=Field_electron_emission&oldformat=true\#Fowler–Nordheim_tunneling}{Fowler-Norheim}
tunneling to charge and discharge a capacitor that keeps it's charge for
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/cdm.pdf}
}
...
lighting bolt through the oxide, it's a similar process.
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/cdm1.pdf}
}
...
Now imagine an IC will hundreds of supplies, and billions of inverters.
...
\section{An HBM ESD zap example}\label{an-hbm-esd-zap-example}\index{An HBM ESD zap example}
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/esd_hbm_model.pdf}
}

\emph{Figure 5: ESD HBM zap example }

Let's simplify and think of the possible permutations, shown in Figure
7. We don't know where the current will enter nor where it will leave
our circuit, so we must make sure that all combinations are covered.


{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/esd_perm_tikz.pdf}
}
...
can use a diode, as shown in Figure 8.
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/esd_zap01_tikz.pdf}
}
...
use a diode, as shown in Figure 9.


{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/esd_zap02_tikz.pdf}
}
...
too small, is the Grounded-Gate-NMOS in Figure 10.


{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/esd_rails_all_tikz.pdf}
}
...
\section{The grounded gate NMOS}\label{the-grounded-gate-nmos}\index{The grounded gate NMOS}

If you try the circuit in Figure 11 with the normal BSIM spice model, it
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/l02_ggnmos.pdf}
}
...
Assume a transistor like the one in Figure 12. The gate, source and bulk
is connected to ground. The drain is connected to a high voltage.


{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/ggnmos.pdf}
}
...
can, when it scatters of the atoms in the depletion zone, knock out an
electron/hole pair.
...
of I/O Buffers in Fully Silicided CMOS Process {[}2{]} for the pretty
pictures you'll get when the drain/source breaks.
...
secondary protection circutis. A resistor further reduces the current,
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/esd.pdf}
}

\emph{Figure 12: Full protection of an input including secondary
protection }

\subsection{Input buffer}\label{input-buffer}\index{Input buffer}
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/fig_methodology.pdf}
}

\emph{Figure 13: Full digital input including Schmitt trigger and level
shifters }

\section{Latch-up}\label{latch-up}\index{Latch-up}
...
Assume we have the circuit in Figure 15. Under certain conditions we can
get a short from VDD to ground.


{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/l02_latchup.pdf}
}

\emph{Figure 14: Inverter that suddenly shorts from VDD to ground
located close to a PAD}

Consider the cross section of the inverter in Figure 16. The latch-up
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/scr_eh.pdf}
}

\emph{Figure 15: Cross section of an inverter }

\subsubsection{Electron injection}\label{electron-injection}
...
This is a phenomena called latch-up. Similar to ESD circuits, latch-up
...
VDD shoots to high, then we can create a good ESD protection circuit.
...
{
\centering
\includegraphics[width=\myfigwidth,height=\myfigheight,keepaspectratio]{media/scr_model.pdf}
}
