...
<small><sub>_Figure 1: Mixed signal design flow, where the analog design is captured both as a schematic for ngspice and as a SystemVerilog model for the digital simulation_</sub></small>
...
<!-- ../media/eventqueue.png -->

![](media/eventqueue.png)


<small><sub>_Figure 2: Flow chart of a digital simulator - advance to the next time step, execute the events scheduled there, and repeat until the queue is empty_</sub></small>
...
<!-- ../media/systemdotnet.png -->

![](media/systemdotnet.png)


<small><sub>_Figure 3: Event queue implemented as a hash table in SystemDotNet, where events at 10, 20, 21 and 30 ns are looked up by time step before being run. Source: the SystemDotNet project on SourceForge_</sub></small>
...
<small><sub>_Figure 4: Gate level netlist of the counter after synthesis, with clk and reset driving eight D flip-flops and the adder logic_</sub></small>
...
<!-- ../media/mixed_simulator.pdf -->

![](media/mixed_simulator.pdf)


<small><sub>_Figure 5: Mixed signal simulator, where a digital and an analog simulator exchange values through DAC and ADC connect modules under a shared event and timestep control_</sub></small>
...
<small><sub>_Figure 6: Top level schematic of the TinyTapeout tt_um_TT06_SAR_wulffern 8-bit SAR ADC, with the SAR core, the output capture block and the power decoupling_</sub></small>




<!-- ../media/tt06_sar_anawave.png -->

![](media/tt06_sar_anawave.png)


<small><sub>_Figure 7: ngspice transient waveforms of the SAR, showing the sampled analog input, the clock and uio_out[0], and the sarp/sarn comparator inputs settling through the bit cycling_</sub></small>
...
<!-- ../media/tt06_sar_anawave2.png -->

![](media/tt06_sar_anawave2.png)


<small><sub>_Figure 8: ngspice transient of the SAR at layout corner, with the ramping analog input, the digital output code, the clock and the uio_out[0] done pulses_</sub></small>
<!-- ../media/tt06_sar_digwave.png -->

![](media/tt06_sar_digwave.png)


<small><sub>_Figure 9: The same behaviour in gtkwave from the SystemVerilog real-number model, where uo_out[7:0] steps as ua_0 ramps_</sub></small>
