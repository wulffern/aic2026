...
<!-- ../media/l4_achai_tikz.pdf -->

![](media/l4_achai_tikz.pdf)


<small><sub>_Figure 1: Analog signal chain from sensor through AFE (amplification, frequency selectivity, domain transfer) to ADC_</sub></small>
...
<!-- ../media/l4_radio_tikz.pdf -->

![](media/l4_radio_tikz.pdf)


<small><sub>_Figure 2: Typical radio receiver frontend: LNA, complex mixer driven by a local oscillator, anti-alias filters and ADCs for the I and Q paths_</sub></small>
...
<!-- ../media/l4_first_order_tikz.pdf -->

![](media/l4_first_order_tikz.pdf)


<small><sub>_Figure 3: Signal flow graph of a general first order filter with coefficients $k_0$, $k_1$ and $\omega_0$_</sub></small>
...
<!-- ../media/l4_biquad_tikz.pdf -->

![](media/l4_biquad_tikz.pdf)


<small><sub>_Figure 4: Signal flow graph of the bi-quadratic (second order) filter_</sub></small>
...
<!-- ../media/l4_gmc_tikz.pdf -->

![](media/l4_gmc_tikz.pdf)


<small><sub>_Figure 5: Single-ended Gm-C integrator: a transconductor driving a capacitor_</sub></small>
...
<!-- ../media/l4_gmc_diff_tikz.pdf -->

![](media/l4_gmc_diff_tikz.pdf)


<small><sub>_Figure 6: Fully differential Gm-C integrator_</sub></small>
...
<!-- ../media/l4_gmc_diff1_tikz.pdf -->

![](media/l4_gmc_diff1_tikz.pdf)


<small><sub>_Figure 7: Differential Gm-C integrator with outputs flipped to implement negative gain_</sub></small>
...
<!-- ../media/l4_gmc1st_tikz.pdf -->

![](media/l4_gmc1st_tikz.pdf)


<small><sub>_Figure 8: First-order Gm-C filter with transconductors $G_{m1}$, $G_{m2}$ and capacitors $C_X$ and $C_A$_</sub></small>
...
<small><sub>_Figure 9: General purpose differential Gm-C biquad with five transconductors_</sub></small>
...
<small><sub>_Figure 10: Transconductor from the master thesis: a differential pair with common-gate cascodes to limit the Miller effect_</sub></small>
...
<!-- ../media/l4_activerc_first_tikz.pdf -->

![](media/l4_activerc_first_tikz.pdf)


<small><sub>_Figure 11: General purpose first-order active-RC filter with parallel RC input and feedback networks_</sub></small>
...
<small><sub>_Figure 12: General purpose active-RC biquad built from two OTAs_</sub></small>
...
<!-- ../media/l4_activerc_tikz.pdf -->

![](media/l4_activerc_tikz.pdf)


<small><sub>_Figure 13: Active-RC integrator: OTA with resistor input and capacitor feedback_</sub></small>
...
Then put $R_{fb1}$ and $R_{fb2}$ back. Each one takes the *first
integrator output* of one path and drives it into the *summing node* of
the other - the same virtual ground the input resistor $R_1$ and the
feedback DAC already inject into. That is the whole of the
cross-coupling, and it is what turns two real modulators into one
complex one.

The figure draws the first stage of the cascade only. What leaves
through $R_3$ and $DAC2$ is the quantization error of this stage, handed
to a second modulator that digitizes it; the digital outputs are then
recombined so the first stage's error cancels. That is what MASH means,
and it is a separate story from the complex part.
...
<small><sub>_Figure 14: One stage of a continuous-time quadrature sigma-delta modulator, both paths drawn: two ordinary real modulators - R1/C1 integrating, Cff1/R2/C2 filtering, ADC1 sampling, DAC1 feeding back, R3 and DAC2 handing off to the second stage - joined by the red cross-coupling that makes the pair complex: each $R_{fb}$ carries one path's first integrator output into the other path's summing node. Inspired by Breems et al. [@breems07], which is fully differential, cascaded, and has a tuning resistor on everything_</sub></small>
...
Both simulations are driven by the same input, a complex tone just
above $f_0$, and you can find it in both spectra as the tall spike. In
the real loop it stands out in the middle of the noise, where the loop
has done nothing for it. In the complex loop it stands in the notch.
The signal did not move; the quiet part of the spectrum did.


<!-- ../media/l04_complex_psd_tikz.pdf -->

![](media/l04_complex_psd_tikz.pdf)


<small><sub>_Figure 15: Simulated output spectra of a first-order sigma-delta loop over the full sample rate. (a) A real loop: the noise notch is symmetric about zero. (b) The same loop made complex: the notch moves to $+f_0$ alone, leaving $-f_0$ noisy - which is what lets a near zero-IF receiver keep the wanted side and throw the image away. The same input tone is present in both, and only in (b) does it land in the notch_</sub></small>
...
<!-- ../media/l04_ota_diff_tikz.pdf -->

![](media/l04_ota_diff_tikz.pdf)


<small><sub>_Figure 16: Differential current mirror OTA_</sub></small>
...
<!-- ../media/l04_ota_vsens_tikz.pdf -->

![](media/l04_ota_vsens_tikz.pdf)


<small><sub>_Figure 17: Common mode sense circuit with source followers and the resistor divider generating the reference $V_{CREF}$_</sub></small>
...
<!-- ../media/l04_ota_vcmfb_tikz.pdf -->

![](media/l04_ota_vcmfb_tikz.pdf)


<small><sub>_Figure 18: Common mode feedback OTA comparing the sensed $V_{COUT}$ to the reference $V_{CREF}$_</sub></small>
...
<!-- ../media/l04_ota_sch.svg -->

![](media/l04_ota_sch.svg)


<small><sub>_Figure 19: CNR_OTA schematic in SKY130: bias, differential OTA, common mode sense (VCM) and common mode feedback OTA_</sub></small>
