...
Temperature chamber, Transfer curve, INL, DNL, Calibration, Allan
deviation, Quantisation, Dither, Dead zone, Random telegraph noise

\section{Two Sensors, Measured}\label{two-sensors-measured}\index{two sensors, measured@Two sensors, measured}
...
{
\centering
\aicfig{media/jnwtt_gr07_tikz.png}
}
...
{
\centering
\aicfig{media/jnwtt_gr06_tikz.png}
}
...
{
\centering
\aicfig{media/jnwtt_transfer_tikz.png}
}
...
{
\centering
\aicfig{media/jnwtt_inl_tikz.png}
}
...
{
\centering
\aicfig{media/jnwtt_cal_gr06_tikz.png}
}
...
{
\centering
\aicfig{media/jnwtt_cal_gr07_tikz.png}
}
...
million of them, and they take \textbf{four distinct values} - in two
...
It is worth being careful about \emph{why} it is still a useful sensor,
...
That matters, because it means nothing accumulates. If the crossing sits
a fraction \(f\) of a cycle past an edge, it sits at the same \(f\) next
time, and the next: the sub-cycle remainder is thrown away each cycle
rather than carried forward. The loop has no memory of it, and that is
why what follows has to be about noise.
...
alternates between \(N\) and \(N+1\) cycles, and the \emph{fraction} of
periods that come out long is an estimate of \(f\). Averaging a million
of them reads that fraction to a few millikelvin.
...
{
\centering
\aicfig{media/jnwtt_deadzone_tikz.png}
}

\emph{Figure 7: GR07's measured noise against where its period happens
to fall between two clock edges, one point per chamber set point. The
noise collapses where the period lands close to a whole number of
cycles}
...
{
\centering
\aicfig{media/jnwtt_dnl_tikz.png}
}

\emph{Figure 8: The same staircase read as a converter: the width of
each fully traversed code against the ideal 4.75 K step. Worst DNL 0.12
LSB, so the steps themselves are even - the problem is their size, not
their regularity}
...
{
\centering
\aicfig{media/jnwtt_allan_tikz.png}
}

\emph{Figure 9: Allan deviation of both sensors over fifteen minutes in
a quiet room. GR07 starts four times more precise, bottoms out at 57 mK
after about eight seconds and then degrades. GR06 keeps improving out
past a minute, to 31 mK}
...
{
\centering
\aicfig{media/jnwtt_spray_tikz.png}
}

\emph{Figure 10: A can of freeze spray at 13 s, then a fingertip on the
package from 88 s to 122 s, both sensors on one capture. The die falls
13 K in 1.4 seconds - a peak rate of 37 K/s. Gaps are dead time between
captures, drawn as gaps rather than interpolated}
...
{
\centering
\aicfig{media/jnwtt_breath_tikz.png}
}

\emph{Figure 11: Four breaths, both sensors, one capture. Each breath
moves both - but GR06 reads about 2.4 times the excursion GR07 does}
...
at ninety seconds of GR06 with the slow dwell drift removed.


{
\centering
\aicfig{media/jnwtt_rts_tikz.png}
}

\emph{Figure 12: Ninety seconds of GR06 at a fixed chamber temperature,
with the slow dwell drift removed. The reading sits flat, drops about
half a kelvin to a second level, stays there a second or two, and comes
back}
...
{
\centering
\aicfig{media/jnwtt_rts_life_tikz.png}
}

\emph{Figure 13: The trap's mean lifetime in the low state against
...
calibration - that is a ninety-second mean. A single reading can be half
a kelvin off whenever the trap happens to be occupied. Unlike GR07's
quantiser this is cheap to fix: a median over a second or two steps
straight over it.
