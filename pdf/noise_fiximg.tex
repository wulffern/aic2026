...
{[}1{]}, the equation specifies that \(SNR = 6.02*Bits +
...
of the auto-correlation function {[}2{]}
...
see {[}3{]}. If we have a multistage system, for example several
amplifiers in cascade, the total noise figure of the system is defined
as
...
chapter 4 in {[}1{]} or chapter 7 in {[}4{]}.
...
different communities, for example {[}5{]} and {[}2{]} where \(S_x(f)\)
is used for both \(S_{x1}(f)\) and \(S_{x2}(f)\). When I started
investigating spectral densities this lead me to believe that different
sources defined the same measure ``spectral density'' in two different
ways. The more sources I investigated the more unsure I was about which
of the two definitions that was correct. After months of searching (not
actively, but sporadically) I eventually found the original source of
the definition of spectral density {[}6{]}. Having the original source
helped, but I still don't know when the original definition split into
the \(S_{x1}\) and \(S_{x2}\) forms above. However, I'm pretty sure it's
just a matter of convenience. To see why the \(S_{x2}\) form is the most
...
\emph{Spectral density is the Fourier Transform of the Auto-Correlation
...
In his 1914 paper {[}6{]} Albert Einstein described, supposedly for the
first time, the auto-correlation function and what we have come to know
as the spectral density. He defined the auto-correlation function as
...
The classic on noise in solid state devices, still the reference {[}2{]}

Friis' original, short and still the clearest statement of why the first
stage decides the noise figure {[}3{]}

Chapter-length treatments of circuit noise, either of which will do
{[}4{]} {[}1{]}

The random walk that thermal noise is made of, from the source {[}6{]}
