\textbf{Keywords:} Statistics, Average Power, PSD, White Noise, Thermal
Noise, SNR, Noise Figure, Friis

\section{Noise}\label{noise}\index{noise@Noise}

Noise is a phenomenon that occurs in all electronic circuits. It places
a lower limit on the smallest signal we can use. Many now have super
audio compact disc (SACD) players with 24bit converters, 24 bits is
around \(2^{24} = 16.78\) Million different levels. If 5V is the maximum
voltage, the minimum would have to be
\(\frac{5V}{2^{24}} \approx 298nV\). That level is roughly equivalent to
the noise in a 50 Ohm resistor with a bandwidth of 96kHz. There exists
an equation that relates number of bits to signal to noise ratio
\citeproc{ref-johns}{{[}1{]}}, the equation specifies that
\(SNR = 6.02*Bits +
...
\section{Statistics}\label{statistics}\index{statistics@Statistics}
...
\[\sigma^2 = \overline{x^2(t)} - \overline{x(t)}^2 \tag{3}\] For a
signal with a mean of zero the variance is equal to the mean square. The
...
\section{Average Power}\label{average-power}\index{average power@Average power}

Average power is defined for a continuous system by (4), and for
discrete samples by (5).

\(P_{av}\) usually has the unit \(A^2\) or \(V^2\), so we have to
multiply/divide by the impedance to get the power in Watts. To get Volts
and Amperes we use the root-mean-square (RMS) value which is defined as
\(\sqrt{P_{av}}\).
...
It is important to realize that it does not matter how you write average
...
you have written. In the remainder of this chapter we will use
\(\overline{e_n^2}\) for average power when we talk about voltage noise
source and \(\overline{i_n^2}\) for average power when we talk about
current noise source. The n subscript is used to identify different
sources and can be whatever.

\phantomsection\label{t:rms}
\begin{longtable}[]{@{}cc@{}}
\toprule\noalign{}
Voltage & Current \\
\midrule\noalign{}
...
\section{Noise Spectrum}\label{noise-spectrum}\index{noise spectrum@Noise spectrum}
...
to denote the PSD. In the same way that we use \(V^2\) as unit of
average power, the unit of the PSD is \(\frac{V^2}{Hz}\) for voltage and
...
of the auto-correlation function \citeproc{ref-ziel}{{[}2{]}}
...
\(\tau=0\) while \(\sin(\omega \tau)\) is asymmetric around
\(\tau = 0\).
...
\[\overline{x^2(t)} = \int_{0}^{\infty}{S_x(f)df} \tag{7}\] which means
we can easily calculate the average power if we know the power spectral
...
\[S_y(f) = S_x(f)\vert H(f)\vert ^2 \tag{8}\] , where \(S_y(f)\) is the
output power spectral density, \(S_x(f)\) is the input power spectral
density and \(H(f)\) is the transfer function of a time-invariant linear
system.
...
pass filter, then (8) tells us that the output spectral density will be
shaped by \(H(f)\). At frequencies above the \(f_x\) in \(H(f)\) we
expect the root power spectral density to fall by 20dB per decade.

\section{Probability Distribution}\label{probability-distribution}\index{probability distribution@Probability distribution}

\textbf{Theorem 1} (Central limit theorem). \emph{The sum of \(n\)
independent random variables subjected to the same distribution will
...
in the real world are Gaussian.\footnote{Gaussian distribution = normal
  distribution. Gaussian describes the amplitude distribution; white
  describes a flat spectral density. Thermal noise happens to be both.}
Take thermal noise for example, it is generated by random motion of
carriers in materials. If we look at a single electron moving through
the material the probability distribution might not be Gaussian. But
summing probability distribution of the random movements with a large
number of electrons will give us a Gaussian distribution, thus thermal
noise is Gaussian.

\section{PSD of a white noise source}\label{psd-of-a-white-noise-source}\index{psd of a white noise source@PSD of a white noise source}
...
\section{Summing noise sources}\label{summing-noise-sources}\index{summing noise sources@Summing noise sources}
...
If \(\overline{e_{1}^2}\) and \(\overline{e_{2}^2}\) are uncorrelated
noise sources we can skip the last term in the sum above and just write

\[\overline{e_{tot}^2} = \overline{e_{1}^2} + \overline{e_{2}^2}\] Most
natural noise sources are uncorrelated.

\section{Signal to Noise Ratios}\label{signal-to-noise-ratios}\index{signal to noise ratios@Signal to Noise Ratios}
...
we don't know the expression for \(\overline{e_{n}^2}\) we can do a FFT
of our output signal. From this FFT we sum spectral components except at
...
\section{Noise figure and Friis
formula}\label{noise-figure-and-friis-formula}
...
is the total output noise. The noise figure is defined as (noise factor
in dB)
...
This brings us right into what is known as Friis formula. The noise
factor definition is only correct at room temperature, for more details,
see \citeproc{ref-friis}{{[}3{]}}. If we have a multistage system, for
example several amplifiers in cascade, the total noise figure of the
system is defined as

\[F = 1 + F_1 - 1 + \frac{F_2 -1}{G_{1}} +
  \frac{F_3-1}{G_{1}G_{2}} + ....\]

Here \(F_i\) is the noise figures of the individual stages and \(G_i\)
is the available gain of each stage. This can be rewritten as
...
\section{Spectral Density}\label{spectral-density}\index{spectral density@Spectral density}
...
chapter 4 in \citeproc{ref-johns}{{[}1{]}} or chapter 7 in
\citeproc{ref-razavi}{{[}4{]}}.

\subsection{Definition of Spectral
Density}\label{definition-of-spectral-density}
...
processing books, like \citeproc{ref-gray.r.m}{{[}5{]}}, is

\[S_{x1}(f) = \int_{-\infty}^{\infty}{R_{x1}(\tau)e^{-j\omega\tau}d\tau}\]
And the one often used in books about noise, like
\citeproc{ref-ziel}{{[}2{]}}, is

\[S_{x2}(f) = 2\int_{-\infty}^{\infty}{R_{x2}(\tau)e^{-j\omega\tau}d\tau}\]
In both cases \(R_{xi}(\tau)\) is the auto-correlation function defined
as
...
\[R_{x1}(\tau) = R_{x2}(\tau)\] This is ok, there is no problem having
two different definitions for two different functions. In reality
\(S_{x1}(f)\) and \(S_{x2}(f)\) are different functions of frequency,
and we could say that

\[S_{x2}(f) = 2S_{x1}(f)\] if the two auto-correlation functions are
equal.

\subsection{Sources of Confusion}\label{sources-of-confusion}\index{sources of confusion@Sources of Confusion}

The problem with spectral density arises when reading literature from
different communities, for example \citeproc{ref-gray.r.m}{{[}5{]}} and
\citeproc{ref-ziel}{{[}2{]}} where \(S_x(f)\) is used for both
\(S_{x1}(f)\) and \(S_{x2}(f)\). When I started investigating spectral
densities this lead me to believe that different sources defined the
same measure ``spectral density'' in two different ways. The more
sources I investigated the more unsure I was about which of the two
definitions that was correct. After months of searching (not actively,
but sporadically) I eventually found the original source of the
definition of spectral density \citeproc{ref-einstein14}{{[}6{]}}.
Having the original source helped, but I still don't know when the
original definition split into the \(S_{x1}\) and \(S_{x2}\) forms
above. However, I'm pretty sure it's just a matter of convenience. To
see why the \(S_{x2}\) form is the most common among sources concerning
noise we look at the inverse Fourier Transform. By the way, if you had
not noticed yet, both forms say that \emph{Spectral density is the
...
Before we proceed lets get rid of the \(e\)'s. We know that
\(e^{j\alpha} =
...
function we can drop the \(j\sin{\omega \tau}\) term. \(S_{x1}(f)\) is
also an even function since the Fourier Transform of an even function is
even.
...
\subsection{Example: Thermal Noise}\label{example-thermal-noise}\index{example: thermal noise@Example: thermal noise}
...
\[S_{th}(f) = 4kTR\] where k is Boltzmann's constant, T the temperature
in Kelvin and R the resistance. But that is the spectral density in the
...
matching definition.\footnote{Note that if you calculate the average
  power of \(S_{th}(f)\) you'll get infinity. You have to include the
  bandwidth of the circuit you are considering for average power to have
  a finite value.}

\subsection{Einstein: The source}\label{einstein-the-source}\index{einstein: the source@Einstein: The source}

In his 1914 paper \citeproc{ref-einstein14}{{[}6{]}} Albert Einstein
described, supposedly for the first time, the auto-correlation function
and what we have come to know as the spectral density. He defined the
...
paper is very short, only 1 page, but it is worth reading. Note that the
spectral density as the Fourier Transform of the auto-correlation
function is often referred to as the \emph{Wiener-Khintchine} theorem.
...
\tightlist
\item
  Noise is random: describe the amplitude by its distribution (usually
  Gaussian) and the power by its spectral density
\item
  Thermal noise is 4kTR in every resistor and kT/C on every sampled
  capacitor - no cleverness removes it
\item
  Flicker noise comes from traps: 1/f density, quieter with bigger
  devices
\item
  Integrate the density over the band you keep to get the power the
  signal must beat
\item
  Friis: with gain up front, only the first stage's noise matters -
  spend your current there
\end{itemize}

\section{Would you like to know
more?}\label{would-you-like-to-know-more}

The classic on noise in solid state devices, still the reference
\citeproc{ref-ziel}{{[}2{]}}

Friis' original, short and still the clearest statement of why the first
stage decides the noise figure \citeproc{ref-friis}{{[}3{]}}

Chapter-length treatments of circuit noise, either of which will do
\citeproc{ref-razavi}{{[}4{]}} \citeproc{ref-johns}{{[}1{]}}

The random walk that thermal noise is made of, from the source
\citeproc{ref-einstein14}{{[}6{]}}
