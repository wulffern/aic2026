...
<!-- ../media/ota_5t_tikz.pdf -->

![](media/ota_5t_tikz.pdf)


<small><sub>_Figure 1: Five transistor OTA_</sub></small>
...
<!-- ../media/ota_cm_tikz.pdf -->

![](media/ota_cm_tikz.pdf)


<small><sub>_Figure 2: Current mirror OTA with mirror ratio $K$_</sub></small>
...
<!-- ../media/ota_two_stage_tikz.pdf -->

![](media/ota_two_stage_tikz.pdf)


<small><sub>_Figure 3: Two stage OTA with Miller compensation_</sub></small>
...
<!-- ../media/ota_folded_tikz.pdf -->

![](media/ota_folded_tikz.pdf)


<small><sub>_Figure 4: Folded cascode OTA_</sub></small>
...
<!-- ../media/ota_inv_tikz.pdf -->

![](media/ota_inv_tikz.pdf)


<small><sub>_Figure 5: The inverter as a transconductor_</sub></small>
...
<!-- ../media/ota_nauta_tikz.pdf -->

![](media/ota_nauta_tikz.pdf)


<small><sub>_Figure 6: Nauta's inverter based transconductor_</sub></small>
...
<small><sub>_Figure 7: Fully differential current mirror OTA_</sub></small>
...
<small><sub>_Figure 8: A common mode feedback amplifier_</sub></small>
...
<small><sub>_Figure 9: Common mode sense circuit with source followers, and the matching resistor divider generating the reference $V_{CREF}$_</sub></small>
...
<!-- ../media/ota_cmfb_sc_tikz.pdf -->

![](media/ota_cmfb_sc_tikz.pdf)


<small><sub>_Figure 10: Switched capacitor CMFB_</sub></small>
...
<!-- ../media/ota_bias_tikz.pdf -->

![](media/ota_bias_tikz.pdf)


<small><sub>_Figure 11: Mirror bias and wide swing cascode bias_</sub></small>
...
<!-- ../media/ota_opamp_tikz.pdf -->

![](media/ota_opamp_tikz.pdf)


<small><sub>_Figure 12: An op amp is an OTA plus an output stage_</sub></small>
...
<small><sub>_Figure 13: The common mode feedback amplifier. VON and VOP are sensed through 60k||20f networks and compared against a 100k/100k mid-supply reference. Each load PMOS is diode connected on its own - the gates are not tied together - so the gain is the modest, well defined $g_{mn}/g_{mp}$ that a common mode loop wants, and the correction leaves as $V_{CMFB}$_</sub></small>
...
<small><sub>_Figure 14: The OTA itself, one side drawn, sized in multiples of the minimum gate length F: a cascoded PMOS tail into the input pair, a cascoded mirror making the first stage output, and a common source second stage with 500 fF of cascode compensation. $V_{CMFB}$ arrives from the amplifier in Figure 13_</sub></small>
...
<small><sub>_Figure 15: The bias generator. The 10 uA reference is mirrored once in an ordinary current mirror - the line along the bottom - and everything above it is wide swing cascode: the narrow 8F12F devices set VCP and VCN, and in each master the mirror device's gate hangs on the far end of its own stack_</sub></small>
